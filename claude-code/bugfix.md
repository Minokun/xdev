...
- [调整]（"改成 X / 不要 Y"）/ [回退]（"撤销 / 回到上一步"）信号处理同 `full-dev.md`，本文件不重复

> **S1 豁免：** S1 路径（≤ 15min 快速修复）无 🔴 硬门禁，Intent Guard 不生效；升级 S2/S3 后立即接入。

> **续航硬约束（S2/S3 生效，同 `full-dev.md` 自动完成不变量）：** 修复未到终止态（ship 完成 + learn 触发判定完毕）前，不得用 "完成" / "已修复" / "下一步继续 X" 等纯文本结束当前轮——TaskList / 状态文件 `next_action` / 后台 shell（如 `uv run pytest -v`）任一显示仍有工作，就必须继续调用工具执行。后台 Bash task（含命令 timeout 自动后台化）是未完成工作，禁止回复"完成后会继续处理结果"即停轮，必须轮询到进程终止再按结果矩阵处理。纯状态问答用 1-2 句回答后立即继续执行剩余队列。完整规则见 `full-dev.md`。
...
| 路径/阶段 | 门禁 | 超限升级 |
|----------|------|----------|
| S1 全量测试 | 1 次 | 升级 S2 调查（S1 是地板层，"不降级"无意义；测试失败即升级调查） |
| S2 调查 | 1 次假设 | 升级 S3 |
| S3 根因 | 3 次假设 | 🔴 暂停 |
