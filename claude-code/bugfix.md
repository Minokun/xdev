...
2. **反向确认（硬规则 2 在 bugfix 下的形态，不可跳过）**：修复完成后，把修复**退回原状**
   （`git stash` / `git checkout -- <file>` / 临时还原那几行），重跑第 1 步的**同一条**命令——
   它必须**重新失败**。仍然通过 → 这条回归测试没有测到你修的东西（假绿），回到第 1 步重写。
   退不回去时（多文件交织修复）写明理由，并在报告中标 `未反向确认`，不得默认它已确认。
3. **同类扫描**：对**刚修好的那个模式**做一次全仓 `rg`，报告"同款 N 处、本次修 M 处"。
   只修实例不清类 = 同一个 bug 换个文件继续活着。实盘：某次修复在 `core/tank.ts` 抓出并修好了
   "测试断言的是程序不引用的死模块"，同类的 `core/powerup.ts` 活到了交付，导致两条验收标准的
   证据链整条建在死模块上。扫出的其余同款：一处一句理由，或开新 issue，**不要顺手全改**（越范围）。
4. 最小修复：只改根因，不顺手重构
5. 跑回归测试到通过；再跑项目约定的全量测试 + lint/build
6. commit：`fix: <root cause>`，正文写 Root cause / Fix 两行
...
- **S2/S3 修复交付检查单（机械，逐项过）**——增量轮的闸门密度不得低于首轮（实盘：修复轮改共享
  取表层自造整片砖墙回归，靠目视截图才发现；2026-09-12 复盘 L4）：
  1. **影响面清单**：`rg -l "<被改符号/被废约定>"` 列出全部使用点，贴进交付报告——
     废除任何隐式约定（负数索引、魔法值这类）时此步强制，漏一个使用点就是一个回归。
  2. 触及**被 ≥2 处复用的共享层**（取表 / 解析 / 输入 / 共享纯函数 / 配置常量）→
     阶段 2 第 3 步的同类扫描必须覆盖第 1 项列出的**全部**使用点，一处一句"已核/已修/理由"。
  3. **变异探针重跑**：项目有探针表时，与本修复所触及文件相关的判据探针 100% 重跑，全红才算数。
  4. **受影响面 E2E / 视觉回归**重跑（涉及 UI 时全量，不只跑新增的复现测试）。
  5. **用户验收基线（L5）**：用户报 bug = 一条负向真值；用户在本轮之前已验收过的行为
     （上一轮"可以玩/可以用"的部分）必须有判据或冒烟检查兜底——没有就补一条再交付。
     增量轮静默破坏已验收行为比漏修 bug 更伤信任（实盘：修复轮破坏了首轮已被目视
     确认的砖墙画面）。
