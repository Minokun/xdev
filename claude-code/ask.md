...
**边界判定**：写图谱（`graphify-out/`）是为答案正确而做，允许；写源码 / 测试 / 构建产物 / git 状态属于执行任务，禁止。发现需要执行任务 → 在 `Next` 建议切 `/xdev:iterate` / `/xdev:bugfix` / `/xdev:full-dev`，由用户决定。
...
- 降级 `rg` + 文件阅读回答
- 在 `Unknowns` 明确："本次结论基于 <数据源>；架构 / 跨模块 / 死代码 类结论准确度受限，若需高准确度请 <具体建议：`graphify update .` / `/xdev:full-dev` 初始化>"

### 步骤 5：query 调用规范（防注入 + 节制）

`$ARGUMENTS` 不可直接拼进 shell。先抽取 1–3 个实体 / 概念关键词作为 query 字符串。
...
- 用户让 `/xdev:ask` **改代码 / 跑测试 / 启动服务 / 部署 / 执行命令** → 停下来，建议切 `/xdev:iterate` / `/xdev:bugfix` / `/xdev:full-dev`，不在 `/xdev:ask` 内动手
- 用户问 "怎么改 / 怎么做 / 如何实现" 属正当问答，不拦截
- **逃生通道（唯一打断自动刷新的方式）**：`/xdev:ask` 默认自动刷新 / 建图（见设计原则）。用户明确说 "快给答案，别刷新" / "别建图" / "用现状回答" → 立即跳过所有 Graphify 写操作，用现有资源回答 + 在 `Unknowns` 说明 "按用户要求未刷新图谱、未建图"
...
| # | 影响 | 维度 | 现象 | 证据 | 建议下一步 |
|---|------|------|------|------|-----------|
| 1 | 高 | 安全热点 | … | `auth/login.ts:42-58` | 切 `/xdev:bugfix` 由其自判 S1/S2/S3 |
| 2 | 中 | 测试缺口 | … | `services/pay.py` 公开 API 无测试 | 切 `/xdev:iterate` 补测试 |
...
**Findings 表的"影响"列只填 高 / 中 / 低**，不得出现 S1 / S2 / S3 字样——严重度分级交给下游 `/xdev:bugfix` / `/xdev:iterate` 自行判断。
