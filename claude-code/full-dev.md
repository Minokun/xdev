...
- 只有全流程终止态达成（阶段 7 发布完成 → 阶段 8 触发条件已判定，触发则跑完 learn、未触发则显式跳过 → 清理状态文件 + audit 目录 + 实施 worktree），或命中本文明确 🔴 暂停条件，才允许最终停轮。
- 后台 shell / Bash task 是未完成工作。收到 `backgroundTaskId`、"Running in the background"、"1 shell still running" 或命令 timeout 后自动后台化时，禁止回复“完成后会继续处理结果”并停轮；必须持续轮询后台输出直到进程终止，然后按当前阶段结果矩阵处理。
...
| 简单功能（< 1 天工作量） | **→ `/superpowers:brainstorming`** | 轻量级头脑风暴，快速产出设计文档 |
...
if [[ "$_CURRENT_BRANCH" =~ ^xdev-full-dev-[0-9]+-[0-9]+$ ]]; then
  ...
  if ! git rev-parse --verify "$_RENAMED" >/dev/null 2>&1; then
    ...
    # worktree 路径随分支变名（git worktree 会自动跟踪，但路径仍是原目录；这里提醒用户）
...
Subagent F → 调用 skill: devex-review（涉及 API / CLI / SDK / 开发者工作流时）
```
...
Subagent C → 调用 skill: health
Subagent D → 调用 skill: devex-review（涉及 API / CLI / SDK / 开发者工作流时）
```
...
汇总：所有 subagent 完成后，先按下面的结果矩阵分类，再决定修复、降级或阻塞；发现本次改动引入的问题立即修复，每个修复单独提交。
...
> **执行顺序（严格）：** ship / land-and-deploy 完成 → 阶段 8 触发判定（需要 worktree 内的 diff）→ 触发则跑 learn → 最后才清理状态文件、audit 目录、worktree。**不得**在阶段 8 之前清理，否则 learn 拿不到 diff 上下文。

---

## 阶段 8：经验沉淀

> **状态更新：** 阶段开始时，更新状态文件「完成阶段」追加 `7`，「当前阶段」改为 `8（经验沉淀）`。在 worktree 内评估；**禁止**此前清理 worktree、状态文件或 audit 目录。
...
## 阶段 8 结束后：清理状态文件 + L3 audit 目录 + 实施 worktree
