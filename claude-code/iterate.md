...
> **续航硬约束（同 `full-dev.md` 自动完成不变量）：** 改动未到终止态（ship 完成）前，不得用 "完成" / "已优化" 等纯文本结束当前轮——TaskList / 状态文件 `next_action` / 后台 shell 任一显示仍有工作，就必须继续调用工具执行。后台 Bash task（含命令 timeout 自动后台化）是未完成工作，禁止回复"完成后会继续处理结果"即停轮，必须轮询到进程终止再按结果矩阵处理。纯状态问答用 1-2 句回答后立即继续执行剩余队列。完整规则见 `full-dev.md`。
