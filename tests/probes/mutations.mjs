...
const PROBES = [
  ...
  { group: 'install', name: 'install.sh 退回硬编码工具清单（G2 回归）', file: 'bin/install.sh',
    old: 'for f in "$src/bin/"*.mjs; do', new: 'for f in "$src/bin/"cost-report.mjs "$src/bin/"drift-check.mjs; do' },
  { group: 'research', name: 'research.md 附录改条款而 gate.js 不改（G3 parity）', file: 'claude-code/research.md',
    old: '③ 统计功效：MDE 论证是否存在且自洽', new: '③ 统计功效：MDE 论证可选' },
  { group: 'gate', name: 'gate.js research 条款改而附录不改（G3 parity 反向）', file: '.claude/workflows/full-dev-gate.js',
    old: '② matrix 每格能否独立复现（独立目录/独立 config/无 run 间隐藏依赖）；', new: '② matrix 每格能否复现；' },
  { group: 'research', name: 'research 门下门丢掉事实性前提例外（G4 回归）', file: 'claude-code/research.md',
    old: '【事实性前提例外】预注册里关于外部世界的断言', new: '【补充说明】预注册里关于外部世界的断言' },
  { group: 'research', name: '目标覆盖从五维裁决里拿掉（G4 回归）', file: 'claude-code/research.md',
    old: '五维裁决：可证伪性 / 可行性 / 可验证性 / 预算 / 目标覆盖。', new: '四维裁决：可证伪性 / 可行性 / 可验证性 / 预算。' },
  { group: 'doc', name: '删除规模档位表（G5 回归）', file: 'claude-code/full-dev.md',
    old: '| **mini** |', new: '| **迷你** |' },
  { group: 'doc', name: 'persona 路由不再提示定档（G5 回归）', file: 'agent.cordis.yml',
    old: '进入后先按技能阶段 0 定档 mini/标准/攻坚', new: '进入后按需处理' },
