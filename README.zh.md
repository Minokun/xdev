...
**一句话**：只保留三类"模型自己骗自己时拿不到真相"的机制，把其他所有条条框框都删了。
...
2. **拿代码对账，不拿记忆对账**——写代码前，计划要过审（门下门：准奏/封驳，封驳强制返工）；写完之后，diff 要对照当初的设计查偏差（drift check）。中间干了什么不重要，**账对不对得上**才重要。
3. **测试必须真跑**——"应该会过"不算过。命令真实执行、输出真实读取、后台任务盯到出结果为止。

为什么值钱：强模型的盲区不会消失，只会换个地方藏。我们的对照实验量化过——最强的审查组合也会漏掉 20% 埋好的缺陷，而一个独立门能抓出已上线代码里的真 bug。xdev 不是教模型干活（它自己会），而是保证**"第二双眼睛"永远在场、错误的代价永远前置**：烂计划在写代码前被封驳（省几天返工），坏改动在合并前被对抗审查拦下（省一次事故）。

另一个隐藏价值：省心。整个流程 196 行 + 5 条硬规则，说人话就是：**"验证必须真做、裁决必须执行、别动主干、危险的事先问我、其他你自己看着办。"** 最后半句是精髓——模型有判断力就让它用，流程只管那些它自己骗自己时会漏的事。
