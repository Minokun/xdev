...
2. 安装 gstack：
   Claude Code：执行 `git clone --single-branch --depth 1 https://github.com/garrytan/gstack.git ~/.claude/skills/gstack && cd ~/.claude/skills/gstack && ./setup`
   Codex/OpenCode/Cursor/Windsurf/其他 agent：执行 `git clone --single-branch --depth 1 https://github.com/garrytan/gstack.git ~/gstack && cd ~/gstack && ./setup --host <agent>`（例如：`./setup --host codex`）
