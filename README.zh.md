...
| **ask** | `/xdev:ask` | `/ask` | 项目问答与潜在问题挖掘（体检模式）；以"答案最新最准"为最高原则 | 1~5 分钟 |
...
完事。`/iterate`、`/map`、`/ask`（rg 模式）已经能用。深度命令（`/full-dev`、`/bugfix`、`/ask` 体检）需要额外 skill，但缺 skill 时 xdev **优雅降级**到能运行的子集，不会崩。
...
| 想用什么 | 需要装 | 累计时间 |
|---------|--------|---------|
| `/iterate` `/map` `/ask`（rg 模式） | **xdev 本体**（必装） | 1 分钟 |
...
| `/ask` 体检 + `/full-dev` 深度架构判断 | + **Graphify**（第 2.6 步；用户已装视为隐式授权 LLM 抽取） | +2 分钟 |
...
> Windsurf 用的是逐文件软链；如果发布说明里有新增工作流文件（如新增 `ask.md`），需重跑 `bash ~/.claude/skills/xdev/bin/install.sh windsurf` 创建新软链。
