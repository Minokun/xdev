...
curl -s -X POST http://127.0.0.1:8000/api/coupons/redeem -H 'Content-Type: application/json' -d '{"user_coupon_id":9001,"order_id":"O20250101001","user_id":1001}' | grep -q '"code":40901' && echo SECOND-REDEEM-REJECTED-OK
pytest tests/test_redeem.py -v -k "second_redeem or idempotent"
```
...
实施要点：核销流程为"校验券状态与有效期，随后将券置为已用"——首先执行 `UPDATE user_coupons SET status='used', used_at=now(), order_id=:order_id WHERE id=:id AND status='unused' AND valid_to >= now()`，即先将券置为已用状态，之后再调用订单服务完成抵扣并取回确认结果；与此同时写入 coupon_flows（action=redeem）。接口幂等：针对同一 order_id 的重复请求直接返回首次核销结果，不再重复执行状态更新。
...
实施要点：计算统一使用 Decimal；折扣的舍入策略采用 ROUND_HALF_UP（四舍五入）。试算接口只负责计算并返回 {discount_amount, final_amount, reason}，不落库、也不改变券状态。门槛判断以订单商品总额为基准；满减券面额大于订单金额时，抵扣额截断至 order_amount - 0.01。测试矩阵覆盖三种券类型 × 满足 / 不满足门槛 × 边界金额（0.01 订单、门槛恰等、面额大于订单额、折扣产生第三位小数）。
